...
def main(config_path):
    ...
    # ## define the model and compile in
    ...
    new_model.compile(loss="sparse",optimizer=OPTIMIZER,metrics=METRICS)


    # ## Train the model
    history = new_model.fit(
        X_train,y_train_bin, # << y_train_bin
        epochs=10,
        validation_data=(X_valid,y_valid_bin), # << y_valid_bin
        verbose=2)

    # ## save the model
    model_dir_path = os.path.join("artifacts","models")
    model_file_path = os.path.join(model_dir_path,"even_odd_base_model.h5")
    new_model.save(model_file_path)

    logging.info(f"base model is saved at {model_dir_path}")
    logging.info(f"evaluation metrics {new_model.evaluate(X_test,y_test_bin)}") #  y_test_bin
